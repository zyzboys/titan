...
class ReporterAgent {
 ...
 private:
  std::string Header() const {
    return "secs_elapsed,interval_qps,logical_space,actual_space,space_amp";
  }
  void SleepAndReport() {
 ...
 public:
  ...
  void FinishedOps(DBWithColumnFamilies* db_with_cfh, DB* db, int64_t num_ops,
    ...
    if (reporter_agent_) {
      ...
      //   std::string blob_space_amp;
      //   db->GetProperty("rocksdb.blob-space-amp", &blob_space_amp);
      //   reporter_agent_->ReportSpaceAmp(blob_space_amp);
      // }
    }
    if (FLAGS_histogram) {
...
// optimization, so skip the check of the max level.
  ...
  void YCSBWorkloadA(ThreadState* thread) {
    ...
    for (int i = 0; i < warm_up_num; i++) {
      ...
      char val_char[1024] = {0};
      sprintf(val_char, "%d", k);      Slice val = Slice(val_char, value_size);
      Status s = db_.db->Put(write_options_, key, val);
      if (!s.ok()) {
        fprintf(stderr, "put error: %s\n", s.ToString().c_str());
        exit(1);
  ...
  void YCSBWorkloadE(ThreadState* thread) {
    ...
    for (int i = 0; i < total; i++) {
      ...
      if (next_op < 95) {
        ...
        bytes += (value_size + key.size()) * scan_size;
